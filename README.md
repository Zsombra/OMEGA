...
> **The toolkit is read-only by construction.** Nothing in `omega/` calls a BattleGrid
> write tool; it emits a submit-ready payload to `out/` and submitting stays a separate,
> human-initiated act.
>
...
| [08 · Strategy Generation](docs/08-strategy-generation.md) | compose a complete validated strategy from a thesis — `wire()` is the exact CREATE request, and the critique states the measured execution profile |
...
| [16 · The write path](docs/16-the-write-path.md) | how to actually write — the schema that refuses its own required key, the first generated plan to compile viable, and the execution surface modelled from measurement |
...
| *Drift (2026-09-09)* | ***144 live** (contract 54.1.0); SWING_HIGH/LOW renamed to DONCHIAN_UPPER/LOWER, nine TPO metrics added, `entry.levelSource` removed. See [22 · Platform drift](docs/22-platform-drift-2026-09-09.md)* |
