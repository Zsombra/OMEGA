...
| 5 | Panel request `category: CRYPTO`, ranked | Refused temporarily ("resolved to no active coins", CRYPTO and ALL) from at least 09:47Z to 09:55Z while ranked L1 and explicit tickers worked; 36 coins at 09:05Z and again at 10:05Z, so no OMEGA capture was lost. The panel falls back to the last known cohort (or `PANEL_SEED_TICKERS`) as an explicit list, labels rows `explicit-fallback`, records `panel.fallback`. Cause not known |
